...
- AE tables can be downloaded as .rtf files.
...
- Step 1: Use `tidy_ae_table()` function (developed in this package) to get a tidy AE table.
- Step 2: Use `tlf_download()` function (developed in this package) to generate a download button If you click this download button, you will download a .zip file, which consists AE summary table with custom filters, for example, serious AE, drug-related AE, etc.
- Step 3: Use `forestly()` function (developed in this package) to generate an interactive forest plot. 
...
db <- tidy_ae_table(population_from  = adsl %>% rename(TRTA = TRT01A),
                    ...
                    listing_interested = define_ae_listing(listing_var = c("SITEID", "USUBJID", "SEX", "RACE", "AGE",  
                                                           ...
                                                           listing_label = c("Site Number", "Participant ID", "Gender", "Race", "Age",
                                                                             "Adverse Event", "Onset Epoch", "Duration", "Duration", 
                                                                             "Intensity", "Serious", "Related", "Action Taken",
                                                                             "Outcome")),
                    ae_listing_label = "Adverse Event" # be consistent with listing_label = ...
                    )
```
